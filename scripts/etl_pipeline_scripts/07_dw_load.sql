/*==============================================================
  DW LOAD SCRIPT - SURROGATE KEY SAFE
  Purpose:
      Load dimensions and facts from clean views into the DW.
      Ensures surrogate key consistency to prevent FK violations.
      Clean views act as the single source of truth.

  Notes:
      - Dimensions must be loaded first
      - Fact tables reference surrogate keys from dimensions
      - Optional: Drop existing tables for clean re-load
      - ETL pipeline: staging → clean → DW
==============================================================*/
...
/*==============================================================
  DW LOAD SCRIPT - SURROGATE KEY SAFE
  Purpose: Load dimensions and facts from clean views into DW
  Notes:
      - All dimensions loaded first
      - Fact tables use surrogate keys from dimensions
      - Prevents FK constraint violations
      - Clean views are the source of truth
==============================================================*/
...
SELECT
    d.DeliveryID,
    r.RouteID,      -- surrogate key
    drv.DriverID,   -- surrogate key
    st.ShipmentTypeID,
    dd.DateKey,
    ds.DeliveryStatusID,
    pf.PriorityFlagID
